...
describe("Ping Pong -- CCTP to CCTP", () => {
  ...
  for (const [pingChainName, pongChainName] of CHAIN_PATHWAYS) {
    ...
    describe(`${pingChainName} <> ${pongChainName}`, () => {
      // Ping setup.
      const pingProvider = new ethers.providers.StaticJsonRpcProvider(LOCALHOSTS[pingChainName]);
      ...
      // Pong setup.
      const pongProvider = new ethers.providers.StaticJsonRpcProvider(LOCALHOSTS[pongChainName]);
      ...
      it(`Ping Network -- Place Market Order`, async () => {
        ...
        const receipt = await pingtokenRouter
          .placeMarketOrder({
            ...
            minAmountOut,
            targetChain,
            redeemer: Buffer.from(tryNativeToUint8Array(pongWallet.address, pongChainName)),
        ...
        const fillVaa = await guardianNetwork.observeEvm(pingProvider, pingChainName, receipt);
      ...
      it(`Pong Network -- Place Market Order`, async () => {
        ...
        const receipt = await pongtokenRouter
          .placeMarketOrder({
            ...
            minAmountOut,
            targetChain,
            redeemer: Buffer.from(tryNativeToUint8Array(pingWallet.address, pingChainName)),
            redeemerMessage: Buffer.from("All your base are belong to us."),
        ...
        const fillVaa = await guardianNetwork.observeEvm(pongProvider, pongChainName, receipt);
