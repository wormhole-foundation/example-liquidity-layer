  ...
  for (const chainName of chainNames) {
    ...
    describe(`Forked Network: ${chainName}`, () => {
      ...
      it("Modify Core Bridge", async () => {
        const coreBridge = IWormhole__factory.connect(await wormholeCctp.wormhole(), provider);
      ...
      it("Modify Circle Contracts", async () => {
        ...
        const messageTransmitter = await circleBridge
          .localMessageTransmitter()
          .then((address) =>
            IMessageTransmitter__factory.connect(address, provider.getSigner(attesterManager))
          );
        ...
        const attester = await circleBridge
          ...
          .then((messageTransmitter) =>
            messageTransmitter.getEnabledAttester(numAttesters.sub(ethers.BigNumber.from("1")))
          );
      ...
      it("Mint CCTP USDC", async () => {
        ...
        const masterMinter = await IUSDC__factory.connect(usdcAddress, provider).masterMinter();
        ...
        {
          const usdc = IUSDC__factory.connect(usdcAddress, provider.getSigner(masterMinter));
