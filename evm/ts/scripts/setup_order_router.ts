...
import { IOrderRouter, RouterInfoStruct } from "../src/types/IOrderRouter";
...
async function addRouterInfo(
  chainId: string,
  orderRouter: IOrderRouter,
  routerInfo: RouterInfoStruct
): Promise<void> {
  console.log(`Adding router info for chain ${chainId}`);
  console.log(routerInfo);
  const tx = await orderRouter.addRouterInfo(chainId, routerInfo);
...
async function main() {
  ...
  const config = getConfig(network, "orderRouter")["routerInfo"];

  // Setup ethers wallet.
  const provider = new ethers.providers.StaticJsonRpcProvider(rpc);
  const wallet = new ethers.Wallet(key, provider);

  // Setup order router contract.
  const orderRouter = IOrderRouter__factory.connect(
    ethers.utils.getAddress(
      tryHexToNativeString(config[chain].endpoint, chain)
    ),
    wallet
  );
  ...
  for (const chainId of Object.keys(config)) {
    ...
    await addRouterInfo(chainId, orderRouter, routerInfo);
  }
}
