import { getConfig, MATCHING_ENGINE_ADDRESS, ZERO_BYTES32, ZERO_ADDRESS } from "./helpers";
...
export function getArgs() {
    const argv = require("yargs").required("network").required("rpc").required("key").argv;
...
async function enableExecutionRoute(
    ...
    if (receipt.status === 1) {
        console.log(`Registration txn succeeded chainId=${chainId}, txHash=${tx.hash}`);
    } else {
...
async function main() {
    ...
    const matchingEngine = IMatchingEngine__factory.connect(
        ethers.utils.getAddress(tryHexToNativeString(MATCHING_ENGINE_ADDRESS, CHAIN_ID_AVAX)),
        wallet
    );

    // Register token routers and set the execution routes.
