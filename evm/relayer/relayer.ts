...
import {IMatchingEngine__factory, ICircleIntegration__factory} from "../ts/src/types";
...
// Supported chains.
const SUPPORTED_CHAINS = [CHAIN_ID_ETH, CHAIN_ID_AVAX, CHAIN_ID_BSC, CHAIN_ID_MOONBEAM];
...
const TESTNET_GUARDIAN_RPC: string[] = ["https://wormhole-v2-testnet-api.certus.one"];
...
function wormholeContract(address: string, signer: ethers.Signer): ethers.Contract {
...
function getBridgeChainId(sender: string): ChainId | null {
    ...
    for (const chainIdString of Object.keys(config)) {
        ...
        const configBridgeAddress = ethers.utils.getAddress(config[chainIdString].bridge);
...
async function executeTokenBridgeOrder(vaa: Uint8Array) {
    try {
        const tx: ethers.ContractTransaction = await MATCHING_ENGINE["executeOrder(bytes)"](
...
function handleRelayerEvent(
    ...
    (async () => {
        try {
            ...
            if (isRedeemed) {
                console.log(`Order already executed, txhash: ${typedEvent.transactionHash}`);
                return;
            }
            ...
            if (isCCTP) {
                ...
                await executeCCTPOrder(receipt, CIRCLE_EMITTER_ADDRESSES[fromChain], vaaBytes);
            } else {
...
function subscribeToEvents(wormhole: ethers.Contract, chainId: SupportedChainId) {
