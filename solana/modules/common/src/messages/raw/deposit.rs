...
pub struct LiquidityLayerDeposit<'a> {
    span: &'a [u8],
...
impl<'a> TryFrom<Payload<'a>> for LiquidityLayerDeposit<'a> {
...
impl<'a> LiquidityLayerDeposit<'a> {
    pub fn span(&self) -> &[u8] {
        self.span
    }

    pub fn deposit(&self) -> Deposit<'a> {
        self.deposit
    }
    ...
    pub fn parse(span: &'a [u8]) -> Result<Self, &'static str> {
        if span.is_empty() {
            return Err("LiquidityLayerDeposit span too short. Need at least 1 byte");
        }
