...
export class WormholeCctpProgram {
    ...
    program: Program<WormholeCctpSolana>;
    ...
    async transferTokensWithPayloadIx(
        ...
        let { payer, sender, mint, srcToken, coreMessage } = accounts;

        const { amount, targetChain, mintRecipient, nonce, payload } = args;
    ...
    async redeemTokensWithPayloadAccounts(
        ...
        const [mint, redeemer] = await splToken
            .getAccount(this.program.provider.connection, redeemerToken)
    ...
    async parseTransactionReceipt(
        ...
        for (const innerIx of txInnerInstructions) {
            // Traverse instructions to find messages posted by the Wormhole CCTP program.
        ...
        return this.program.provider.connection
            ...
            .then((infos) =>
                infos.map((info, i) => {
                    ...
                    const nonce = DepositWithPayload.decode(payload).deposit.cctpNonce;
